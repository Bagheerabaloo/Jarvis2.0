...
@dataclass
class FunctionEditPag(QuotesFunction):
    name: str = 'edit_pag'
    ...
    async def state_3(self):
        new_pag = self.message.last_message()
        self.telegram_function.settings["new_pag"] = new_pag
        text = "Confirm?"
        keyboard = [["Yes", "No"]]
        await self.send_message(chat_id=self.chat.chat_id,
                                text=text,
    ...
    async def state_4(self):
        ...
        if action == "Yes":
            ...
            new_pag = int(self.telegram_function.settings["new_pag"])
            self.postgre_manager.update_note_by_note_id(note_id=note_id, set_params={"pag": new_pag})
            ...
            await self.send_message(chat_id=self.chat.chat_id, text="Page updated")
        else:
            await self.edit_message(chat_id=self.chat.chat_id, text="Confirm?\n\n*No*", parse_mode="Markdown")
            await self.send_message(chat_id=self.chat.chat_id, text="Page not updated")
